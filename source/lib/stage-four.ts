...
export class StageFour extends Construct {
    ...
    constructor(scope: Construct, id: string, props: StageFourProps) {
        ...
        const defaultCalculateTreehashPolicy = calculateTreehashRole.node.findChild('DefaultPolicy').node.defaultChild as CfnResource;
        ...
        copyToDestinationBucketRole.addToPrincipalPolicy(iamSec.IamPermissions.lambdaLogGroup(`${Aws.STACK_NAME}-copyToDestinationBucket`));
        ...
        copyToDestinationBucket.addEventSource(new eventsource.SqsEventSource(copyToDestinationBucketQueue, { batchSize: 1 }));
