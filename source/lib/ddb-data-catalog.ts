...
export class DynamoDataCatalog extends Construct {
    ...
    constructor(scope: Construct, id: string) {
        ...
        statusTable.autoScaleWriteCapacity({ minCapacity: 30, maxCapacity: 500 }).scaleOnUtilization({
        ...
        statusTable.autoScaleReadCapacity({ minCapacity: 25, maxCapacity: 500 }).scaleOnUtilization({
            targetUtilizationPercent: TARGET_UTILIZATION,
            scaleOutCooldown: Duration.seconds(SCALING_OUT_COOLDOWN_SEC),
            scaleInCooldown: Duration.seconds(SCALING_IN_COOLDOWN_SEC),
        });
    ...
    static configureGsiAutoScaling(
        table: dynamo.Table,
        indexName: string,
