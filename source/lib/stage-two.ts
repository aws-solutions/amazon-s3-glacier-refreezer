...
export class StageTwo extends Construct {
    ...
    constructor(scope: Construct, id: string, props: StageTwoProps) {
        ...
        deployGlueJobScriptRole.addToPrincipalPolicy(
            iamSec.IamPermissions.lambdaLogGroup(`${Aws.STACK_NAME}-deployGlueJobScript`)
        );
        ...
        requestArchivesRole.addToPrincipalPolicy(
        ...
        requestArchivesRole.addToPrincipalPolicy(
            iamSec.IamPermissions.lambdaLogGroup(`${Aws.STACK_NAME}-requestArchives`)
        );
        requestArchivesRole.addToPrincipalPolicy(iamSec.IamPermissions.glacier(props.glacierSourceVault));

        const defaultRequestArchivesPolicy = requestArchivesRole.node.findChild("DefaultPolicy").node
        ...
        CfnNagSuppressor.addCfnSuppression(
            <CfnResource>glueRole.node.defaultChild,
            "W28",
            "Transient, one off solution - updates must be through deletion/redeployment of the stack only"
        );
        ...
        glueRole.addToPolicy(
            new iam.PolicyStatement({
                ...
                resources: [`arn:${Aws.PARTITION}:logs:${Aws.REGION}:${Aws.ACCOUNT_ID}:log-group:/aws-glue/jobs/*:**`],
            })
        );
