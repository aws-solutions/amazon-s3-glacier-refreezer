...
export class AmazonS3GlacierRefreezerStack extends Stack {
    constructor(scope: Construct, id: string, props: SolutionStackProps) {
            ...
            "AWS::CloudFormation::Interface": {
                ...
                ParameterLabels: {
                    ...
                    [cloudtrailExportConfirmation.logicalId]: {
                        default:
                            "Have you checked that there is only one Cloudtrail export to S3 bucket configured in your account?",
                    },
                    [snsTopicForVaultConfirmation.logicalId]: {
                        default:
                            "Has default SNS notification topic on the vault been disabled or is it acceptable to receive notification for ALL archives in the vault?",
                    },
                },
            },
        };
        ...
        archiveNotificationTopic.addToResourcePolicy(
            iamSec.IamPermissions.snsGlacierPublisher(archiveNotificationTopic)
        );
        archiveNotificationTopic.addToResourcePolicy(
            iamSec.IamPermissions.snsDenyInsecureTransport(archiveNotificationTopic)
        );
