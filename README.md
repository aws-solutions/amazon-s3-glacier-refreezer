...
Amazon S3 Glacier Vault based customers who wanted to save further costs on storage.
...
Amazon S3 Glacier Vault’s archives to a configurable target Amazon S3 bucket.
...
The solution utilizes the following as inputs for deployment: the source Glacier Vault name, the destination Amazon S3 bucket name and storage class, and the Glacier retrieval type.

#### Stage One: Get Inventory

Deployment process starts the process automatically by running Request Inventory Lambda function that will check the input parameters and make sure that destination bucket and the glacier vault are accessible. Then it will request the latest Glacier Vault inventory report as the source list of Glacier Vault archives that need to be copied to destination Amazon S3 bucket and storage class. 
...
Configuration:
- Amazon S3 Glacier Source Vault
...
> **The solution will collect anonymously the following four data points: Region, Target Storage Class, Archive Count and Vault Size.**
